...
| 项 | 值 |
| --- | --- |
| 包名 | `dsh-devbuddy-reviewer` |
| Node 入口 | `exports["."]` → `lib/index.js` |
| 浏览器入口 | `exports["./client"]` → `lib/client.js`（由 `/plugins/dsh-devbuddy-reviewer/client.js` 提供） |
| cordis 插件名 | `dsh-devbuddy-reviewer` |
| bundle id | `devbuddy-reviewer` |
| API 前缀 | `/api/devbuddy`（左栏 `protocol.ts` 已为右栏实现预留此前缀） |
| dsh 引擎 | `>=0.1.5-rc.2` |
...
```yaml
- insert:
    - id: devbuddy-reviewer
      name: 'dsh-devbuddy-reviewer'
```
...
flowchart TD
    ...
    A --> C[sidebarRight.registerPanel<br/>id: devbuddy-reviewer]
...
| `routes.ts` | `/api/devbuddy/*` 精确路由；loopback + same-origin 信任围栏（复用左栏 `isTrusted` 模式） |
...
sequenceDiagram
    ...
    L->>S: GET/POST /api/devbuddy-left/node（保存文档）
    S->>D: 写入节点文件
    Note over R: 用户在文档中选中文本发起 Review
    R->>S: POST /api/devbuddy/reviews（锚点 + 内容）

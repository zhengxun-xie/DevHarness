...
/**
 * DevDelivery standalone plugin build — two artifacts from one tsdown invocation:
 *
 *   1. lib/index.js  — Node ESM, the host half (registered via cordis.patch.yml).
 *      Serves /api/devdelivery/* loopback JSON routes; no runtime npm deps.
 ...
 * document.head (scoped by the `ddl-` prefix), so no CSS-module pipeline is
 * needed outside the DSH workspace build.
 */

const PLUGIN_ID = 'dsh-devdelivery'

...
sequenceDiagram
    ...
    U->>C: Submit Review
    C->>H: POST /api/devreviewer/reviews
...
| 入口 | 行为 |
| --- | --- |
| 左栏**编辑态** textarea 选区 | 选区稳定后在选区 rect **正上方**原位弹「添加评论」浮框；同时在覆盖层绘制待提交虚线锚点（失焦不消失）。点击浮框直接 `openTab('devreviewer', { params })`，params 扩展携带 `draftAnchor`，右栏直接进入 Composer。不再广播 `DEVBUDDY_SELECTION` 让右栏显示提示条 |
...
1. **按文档聚合**：同一文档的评审归入一个 `.dbr-doc-group`，组头始终渲染（即使该组折叠）——**文件名 basename**（如「ProjectInfo.md」「Intent.md」，不是左栏流程节点名）+ 计数；组头前有随折叠态旋转的 chevron 视觉指示。不同文档的组之间渲染一条分隔线（`.dbr-doc-group + .dbr-doc-group`）。
...
14. **按文档聚合**：列表按左栏节点顺序分组，组头「文件名 + 计数」（ProjectInfo.md / Intent.md 等 basename，非流程节点名）且始终渲染，组间有分隔线，卡片不重复文件名；组内严格按文档行序排列；

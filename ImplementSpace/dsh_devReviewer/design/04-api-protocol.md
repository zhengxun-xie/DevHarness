...
- Base URL：`/api/devreviewer`（左栏 [protocol.ts](../../dsh_devbuddy/src/protocol.ts) 注释中已为右栏预留此前缀，与 `/api/devreviewer-left` 区分，两插件共存）；
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/devreviewer/projects` | 当前可评审项目列表（读共享注册表） |
| POST | `/api/devreviewer/project/activate` | 设置右栏当前项目 |
| GET | `/api/devreviewer/reviews` | Review 列表（索引 + 筛选） |
| GET | `/api/devreviewer/review` | 单条 Review 详情（含正文线程） |
| POST | `/api/devreviewer/reviews` | 创建 Review（Inline Review 提交） |
| POST | `/api/devreviewer/review/append` | 追加讨论条目 |
| POST | `/api/devreviewer/review/transition` | 状态迁移 |
| POST | `/api/devreviewer/review/remove` | 删除（仅 open/rejected/duplicated 且 author 本人） |
| GET | `/api/devreviewer/document` | 取文档内容 + 评审锚点解析结果 |
| POST | `/api/devreviewer/agent/send` | Send to Agent（见 07） |
| GET | `/api/devreviewer/agent/context` | 预览将装配的 Agent Context（dry-run） |
...
| 数据 | 归属端点 |
| --- | --- |
| 项目注册表 CRUD、节点文档读写 | `/api/devreviewer-left/*`（不重复实现） |
| Review 全部读写、状态、Agent 下发 | `/api/devreviewer/*` |

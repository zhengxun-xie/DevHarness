...
> DevReviewer 插件（包名 `dsh-devreviewer`）是 DevBuddy 的右侧栏评审插件，承载以 **Review 为驱动**的人机协作工作流。
...
| 维度 | 左栏主插件 `dsh-devbuddy-left` | 右栏 Reviewer 插件 `dsh-devreviewer` |
| --- | --- | --- |
| 位置 | dsh 左侧边栏 | dsh 右侧边栏（会话右栏的 panel 席位） |
| 职责 | 多项目管理、工作流节点文档编辑 | 评审的创建、浏览、流转、发送给 Agent |
| API 前缀 | `/api/devreviewer-left/*` | `/api/devreviewer/*`（左栏 protocol 中已预留） |
| 共享 | 项目注册表 `~/.dsh/devbuddy/registry.json`、项目目录与节点文件约定 | 同左 |
| bundle | `devbuddy-left` | `devreviewer` |

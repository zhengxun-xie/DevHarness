/**
 * Browser half of the DevReviewer loopback API. Same-origin fetch
 * against the exact routes the host half registered at /api/devreviewer/*.
 *
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  const response = await fetch(`${DEVREVIEWER_API_PREFIX}${path}`, {
    cache: 'no-store',
    ...init,
    headers: { 'content-type': 'application/json', ...(init?.headers ?? {}) },
  })
  const body = await response.json().catch(() => ({})) as T & { error?: string }
  if (!response.ok) throw new Error(body.error ?? `devreviewer request failed: ${response.status}`)
  return body
}
